...
public class TriggerExistingEventFunction implements NamedJavaFunction {
	private static final String FUNCTION_NAME = "triggerExistingEvent";
	...
	public int invoke(LuaState L) {
		...
		return triggerExistingEvent(L);
	}
	
	/**
	 * Method which implements the function logic. The parameters in the stack should be:
	 * 0: String event_id
	 * 1: String triggered_by (optional). "" if empty.
	 */
	public int triggerExistingEvent(LuaState L) {
		InfantiumResponse resp = InfantiumResponse.EmptyField;
		
		try {
			String event_id = L.checkString(1);
			String triggered_by = L.checkString(2);

			if(triggered_by.equals("")) {
				resp = LuaLoader.infantium.triggerExistingEvent(event_id);

			} else {
				resp = LuaLoader.infantium.triggerExistingEvent(event_id, triggered_by);
